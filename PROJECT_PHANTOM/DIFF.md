...
| File criteria keywords "written" and "changed" added to `criteria.py` but never tested | Two new `test_phantom.py` checks: `criteria.py written` and `test_phantom.py changed` — all 8 trigger words (updated, changed, done, committed, created, documented, added, written) now have dedicated tests (588 → 590) |
...
| "done" and "committed" file trigger words in `criteria.py` had no tests — only "updated/created/documented/added/written/changed" were tested | Two more `test_phantom.py` checks: `ANTI_DRIFT.md done` and `drift_guard.py committed` — all 8 trigger words now covered (591 → 593) |
