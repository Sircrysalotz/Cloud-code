...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Use session_start_ref from phantom session state as diff base "
             "(reads PHANTOM_STATE env var or /tmp/phantom_session.json)"
    )
    p.add_argument(
        "--state-file", default=None,
        help="Path to phantom session state file (overrides PHANTOM_STATE)"
    )
    p.add_argument(
...
def read_session_start_ref(state_file: str | None = None) -> str | None:
    """Read session_start_ref from phantom session state."""
...
def run(args):
    ...
    if since is None and getattr(args, "session", False):
        ...
        if session_ref:
            ...
            print("WARNING: --session set but no session_start_ref found. Falling back to auto.",
                  file=sys.stderr)
    if since is None:
    ...
    if not args.quiet:
        print(f"Coverage check — diff since: {since}{session_label}  |  {done}/{total} targets touched ({pct:.0f}%)")
        print()
